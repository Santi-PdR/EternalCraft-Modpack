...
const mockManifest = {
  schema:2,version:'1.0.0',releaseName:'Siege Origin',minecraft:'1.20.1',forge:'47.4.10',minimumLauncher:'0.65.39',files:[],remove:[],
...
function renderUpdateCenter(){
  ...
  if($('updatesLauncherTitle'))$('updatesLauncherTitle').textContent=launcherNeeds?'Nueva versión disponible':`v${appState?.appVersion||'0.65.39'}`;
...
function bind(){
  ...
  $('checkPackBtn').addEventListener('click',()=>refreshPack(true).catch(()=>{}));$('quickCheck').addEventListener('click',()=>refreshPack(true).catch(()=>{}));$('updatePackBtn').addEventListener('click',()=>runPackAction('update'));$('openInstanceBtn').addEventListener('click',()=>api.openInstance());
