...
const mockManifest = {
  schema:2,version:'1.0.0',releaseName:'Siege Origin',minecraft:'1.20.1',forge:'47.4.10',minimumLauncher:'0.61.0',files:[],remove:[],
...
function applyVisuals(config={}){
  ...
  const radius=Number(config.cardRadius); document.documentElement.style.setProperty('--radius-lg',`${clamp(Number.isFinite(radius)?radius:16,8,28)}px`);
  const names=['frontline','night','canyon','anniversary','cyborg','dummies','orbit','laststand','vought','nightop','rooftop','tempest','urban'];
...
function renderUpdateCenter(){
  ...
  if($('updatesLauncherTitle'))$('updatesLauncherTitle').textContent=launcherNeeds?'Nueva versión disponible':`v${appState?.appVersion||'0.61.0'}`;
