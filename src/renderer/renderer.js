...
const mockManifest = {
  schema:2,version:'1.0.0',releaseName:'Siege Origin',minecraft:'1.20.1',forge:'47.4.10',minimumLauncher:'0.65.40',files:[],remove:[],
...
// Button handlers must consume rejected IPC promises locally.  A shell/open
// operation can legitimately fail (missing folder, closed file manager or a
// denied external URL); letting that rejection escape reaches the global
// error handler and makes an otherwise healthy page look broken.
function runUiAction(action){
  try { return Promise.resolve(typeof action === 'function' ? action() : action).catch(err => toast(err?.message || String(err), 'error')); }
  catch (err) { toast(err?.message || String(err), 'error'); return Promise.resolve(null); }
}
function reportUnexpectedError(prefix,error){
  ...
  if(active) showPageRuntimeError(active.id.replace(/^page-/,''),detail);
}
...
function showPageRuntimeError(page,detail){
  const host=$(`page-${page}`); if(!host)return;
  let banner=host.querySelector('.page-runtime-error');
  if(!banner){
...
function setPage(page){
  ...
  if(page==='updates'){
    ...
    refreshUpdateCenter(false).catch(err=>{
      ...
      showPageRuntimeError('updates',err?.message||err);
      // Keep the failure local to this page; the rest of the launcher remains usable.
      void err;
    });
  }
  if(page==='gallery')refreshClips(false).catch(err=>showPageRuntimeError('gallery',err?.message||err));
  if(page==='mods')refreshMods(false).catch(err=>showPageRuntimeError('mods',err?.message||err));
  if(page==='modpack')refreshPack(false).catch(err=>showPageRuntimeError('modpack',err?.message||err));
...
function renderDiscordChannels(){
  ...
  for(const channel of channels){const row=document.createElement('button');row.className='connectivity-row';row.type='button';row.innerHTML=`<span><b>${escapeHtml(channel.name||'Discord')}</b><small>Canal oficial</small></span><strong>ABRIR ↗</strong>`;row.addEventListener('click',()=>runUiAction(()=>api.openExternal(channel.url)));host.appendChild(row);}
}
...
async function refreshClips(showToast=false){
  if(!api.clipsList)return null;try{clipsState=await api.clipsList();renderClips(clipsState);if(showToast)toast(`${clipsState.total||0} archivos encontrados.`,'success');return clipsState;}catch(err){if(showToast)toast(err.message||String(err),'error');if($('page-gallery')?.classList.contains('active'))showPageRuntimeError('gallery',err?.message||err);return null;}
}
...
function fillBaseState(state){
  ...
  $('packMc').textContent=manifest.minecraft||config.minecraft.version||'—'; $('packForge').textContent=manifest.forge||config.minecraft.forgeVersion||'—'; $('packChannel').textContent=state.manifestStale?'CACHÉ':state.manifestSource==='remote'?'GITHUB':state.manifestSource==='development'?'LOCAL':'SIN PUBLICAR'; $('packAutoUpdate').textContent=config.pack.autoUpdate?'ON':'OFF';
...
async function refreshMods(showToast=false){
  ...
  refreshMods.inFlight=(async()=>{try{const state=await api.listMods();renderMods(state);if(showToast)toast('Lista de mods actualizada.','success');return state;}catch(err){if(showToast)toast(err.message||String(err),'error');if($('page-mods')?.classList.contains('active'))showPageRuntimeError('mods',err?.message||err);return null;}})();
...
async function refreshRecovery(){try{return renderRecovery(await api.listRecoveryPoints())}catch(err){if($('recoveryList'))$('recoveryList').innerHTML='<div class="empty-state">No pude leer los puntos de restauración.</div>';if($('page-modpack')?.classList.contains('active'))showPageRuntimeError('modpack',err?.message||err);return null;}}
...
function renderUpdateCenter(){
  ...
  if($('updatesLauncherTitle'))$('updatesLauncherTitle').textContent=launcherNeeds?'Nueva versión disponible':`v${appState?.appVersion||'0.65.40'}`;
...
async function applyAllUpdates(){
  try{
    ...
    // removed external catalogue, so this branch must never call a stale helper.
    if(modCount)toast('Los mods personales se mantienen locales y no tienen un catálogo de actualización automático.','info');
    await refreshUpdateCenter(false);toast('Actualizaciones de contenido completadas.','success');
  }catch(err){toast(err.message||String(err),'error');}
}
...
async function runPackAction(mode='update'){
  ...
  catch(err){toast(err.message||String(err),'error');return false;}
  finally{hideOperation();setBusy(false)}
}
...
function saveSettings(silent=false){
  // Serialize writes so a slow IPC response cannot apply an older snapshot
  // after a newer settings change.
  const run=()=>saveSettingsNow(silent);
  const next=settingsSaveTail.then(run,run);
  settingsSaveTail=next.catch(()=>{});
  return next;
}
async function saveSettingsNow(silent=false){
...
function bind(){
  ...
  $$('.nav-item').forEach(btn=>btn.addEventListener('click',()=>{setPage(btn.dataset.page);if(btn.dataset.page==='mods')refreshMods(false);if(btn.dataset.page==='modpack')refreshRecovery();if(btn.dataset.page==='updates')refreshUpdateCenter(false).catch(()=>{});if(btn.dataset.page==='support'){refreshCrashGuard(false).catch(()=>{});refreshChangeHistory().catch(()=>{})}}));
  $$('[data-page-jump]').forEach(btn=>btn.addEventListener('click',()=>setPage(btn.dataset.pageJump)));
  $('minimizeBtn').addEventListener('click',api.minimize);$('maximizeBtn').addEventListener('click',api.toggleMaximize);$('closeBtn').addEventListener('click',api.close);$('operationMinimizeBtn')?.addEventListener('click',minimizeOperation);$('operationTaskPill')?.addEventListener('click',restoreOperation);$('sidebarCollapseBtn')?.addEventListener('click',async()=>{const collapsed=!document.body.classList.contains('sidebar-collapsed');document.body.classList.toggle('sidebar-collapsed',collapsed);const button=$('sidebarCollapseBtn');button?.setAttribute('aria-expanded',String(!collapsed));if(button){button.title=collapsed?'Expandir barra lateral':'Contraer barra lateral';button.setAttribute('aria-label',button.title);}try{const cfg=await api.saveSettings({launcher:{sidebarCollapsed:collapsed}});if(appState)appState.config=cfg}catch(_){}});$$('[data-settings-tab]').forEach(btn=>btn.addEventListener('click',()=>{queueSettingsSave();if($('settingsSearch'))$('settingsSearch').value='';setSettingsGroup(btn.dataset.settingsTab);applySettingsSearch();}));
  ...
  $('saveDiagnosticBtn')?.addEventListener('click',async()=>{try{const file=await api.saveDiagnostic();if(file)toast('Informe de soporte guardado.','success')}catch(err){toast(err.message||String(err),'error')}});$('supportBundleBtn')?.addEventListener('click',async()=>{try{const file=await api.exportSupportBundle();if(file)toast('Paquete de soporte guardado. No incluye contraseñas, API keys ni mundos.','success')}catch(err){toast(err.message||String(err),'error')}});$('openLogsBtn').addEventListener('click',()=>runUiAction(()=>api.openLogs()));
