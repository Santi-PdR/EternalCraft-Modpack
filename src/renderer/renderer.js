...
const mockConfig = {
  ...
  mods:{allowUserMods:true,sort:'recent',provider:'modrinth',category:'all',environment:'all',releaseChannel:'release',autoCheckUpdates:true,autoUpdateUserMods:false,compatibilityWarnings:true,protectServerCompatibility:true,autoChangeSnapshots:true,curseforgeProxyUrl:''},
...
function toast(message,type=''){
  ...
  if(text){notificationHistory.unshift({message:text,type:type||'info',time:new Date().toISOString()});notificationHistory=notificationHistory.slice(0,30);renderNotifications();}
}
function renderNotifications(){
  const host=$('notificationList'),badge=$('notificationsBadge');if(!host||!badge)return;
  badge.textContent=String(notificationHistory.length);badge.classList.toggle('hidden',notificationHistory.length===0);host.innerHTML='';
...
function toggleNotifications(force){const drawer=$('notificationDrawer');if(!drawer)return;const show=typeof force==='boolean'?force:drawer.classList.contains('hidden');drawer.classList.toggle('hidden',!show);}
...
function fillBaseState(state){
  ...
  const presetLabels={performance:'RENDIMIENTO',balanced:'EQUILIBRADO',quality:'CALIDAD'};const displayInfo=state.system?.display||{};const gpuNames=(state.system?.gpus||[]).map(g=>g.name).filter(Boolean);
  if($('homePresetBadge'))$('homePresetBadge').textContent=presetLabels[config.minecraft.preset]||'EQUILIBRADO';if($('homeProfileRam'))$('homeProfileRam').textContent=`${Math.round((config.minecraft.maxMemoryMb||6144)/1024)} GB`;if($('homeProfileResolution'))$('homeProfileResolution').textContent=config.minecraft.useSystemResolution!==false?`${displayInfo.width||config.minecraft.width||1920}×${displayInfo.height||config.minecraft.height||1080} · AUTO`:`${config.minecraft.width||1920}×${config.minecraft.height||1080}`;if($('homeProfileGpu'))$('homeProfileGpu').textContent=config.minecraft.preferDedicatedGpu!==false?(gpuNames[0]||'PREFERIDA'):'AUTOMÁTICA';if($('homeProfileJava'))$('homeProfileJava').textContent=java?.found?`JAVA ${java.major||17}`:'AUTO';
  ...
  const javaOk=Boolean(java?.found&&java?.major===17); const javaManaged=Boolean(java?.managed); $('javaReadout').textContent=javaOk?`Java ${java.version} · ${javaManaged?'administrado por Eternal Craft':'detectado en el sistema'}`:`Java 17 no encontrado · se puede preparar automáticamente al jugar`; $('installJavaBtn').textContent=javaOk?'JAVA 17 LISTO':'INSTALAR JAVA 17'; $('installJavaBtn').disabled=javaOk;
...
function renderReadiness(){
  if(!appState)return; const javaOk=Boolean(appState.java?.found&&appState.java?.major===17)||appState.config.minecraft.autoInstallJava!==false,packOk=Boolean(packState?.healthy),serverOk=Boolean(serverState?.online),launcherOk=appState.launcherCompatible!==false;
...
async function refreshModAudit(showToast=false){
  ...
  try{
    ...
    bar.classList.toggle('bad',bad>0);bar.classList.toggle('warn',bad===0&&warn>0);bar.classList.toggle('ok',bad===0&&warn===0);
...
async function searchCatalog(append=false,showBusy=true){
  ...
  try{const result=await api.searchMods({provider,query,category,environment,releaseChannel,sort,offset:catalogOffset,limit:30});const results=result.results||[];if(append){const merged=[...catalogState];const ids=new Set(merged.map(x=>`${x.provider}:${x.id}`));for(const item of results)if(!ids.has(`${item.provider}:${item.id}`))merged.push(item);renderCatalog(merged,result.configured!==false,provider,result.reason||'');}else renderCatalog(results,result.configured!==false,provider,result.reason||'');catalogOffset+=results.length;catalogHasMore=results.length>=30;$('catalogLoadMoreBtn')?.classList.toggle('hidden',!catalogHasMore);$('catalogNotice').innerHTML=provider==='modrinth'?'<b>Modrinth</b> · Forge 1.20.1 · resultados compatibles y carga progresiva.':'<b>CurseForge</b> · usa la API oficial cuando el desarrollador configura su acceso.';}
...
async function installCatalogMod(project){
  ...
  try{
    let plan=null;
    if(project.provider==='modrinth'&&api.planModInstall){plan=await api.planModInstall(project);if(plan.blocked&&appState?.config?.mods?.protectServerCompatibility!==false){toast(plan.warnings?.[0]?.text||'Este mod no es compatible con el cliente.','error');return;}const deps=plan.dependencies?.length||0;const warn=(plan.warnings||[]).map(x=>`• ${x.text}`).join('\n');const msg=[`${project.name||'Este mod'} agregará ${plan.needed?.length||1} archivo(s)${deps?` · ${deps} dependencia${deps===1?'':'s'}`:''}.`,`Descarga aproximada: ${formatBytes(plan.totalSize||0)} · canal ${String(plan.releaseChannel||'release').toUpperCase()}.`,warn].filter(Boolean).join('\n\n');if(!await askConfirm({title:'PLAN DE INSTALACIÓN',message:msg,confirmText:'Instalar'}))return;}
...
function renderTestDiff(result){
  ...
  for(const item of candidates.sort((a,b)=>new Date(b.modifiedAt)-new Date(a.modifiedAt))){const row=document.createElement('div');row.className='test-diff-row';row.innerHTML=`<span class="test-diff-type">${item.type}</span><div><b></b><small></small></div><button class="ghost">PROMOVER A SIEGE</button>`;row.querySelector('b').textContent=item.name;row.querySelector('small').textContent=`${formatBytes(item.size||0)}${item.modifiedAt?` · ${timeAgo(item.modifiedAt)}`:''}`;row.querySelector('button').addEventListener('click',()=>promoteTestMod(item.name));host.appendChild(row);}
}
...
function renderUpdateCenter(){
  ...
  if($('updatesLauncherTitle'))$('updatesLauncherTitle').textContent=launcherNeeds?'Nueva versión disponible':`v${appState?.appVersion||'0.25.0'}`;
  ...
  const javaOk=Boolean(appState?.java?.found&&appState?.java?.major===17);if($('updatesRuntimeTitle'))$('updatesRuntimeTitle').textContent=javaOk?`Java ${appState.java.version||17}`:'Java 17 pendiente';if($('updatesRuntimeText'))$('updatesRuntimeText').textContent=javaOk?(appState.java.managed?'Runtime administrado por Eternal Craft.':'Runtime detectado en el sistema.'):'Se puede instalar automáticamente.';
...
function updatePlayAvailability(){
  if(!$('playBtn')||!appState)return; const javaOk=Boolean(appState.java?.found&&appState.java?.major===17),javaAuto=appState.config.minecraft.autoInstallJava!==false,configured=Boolean(appState.manifestConfigured),compatible=appState.launcherCompatible!==false,healthy=packState?.healthy;
  ...
  $('playBtn').disabled=disabled; $('playCaption').textContent=caption;
}
...
async function saveSettings(){
  ...
  try{const config=await api.saveSettings(patch);appState.config=config;fillBaseState({...appState,config});$('settingsSaved').textContent='GUARDADO';setTimeout(()=>$('settingsSaved').textContent='',2200);toast('Ajustes guardados.','success');}catch(err){toast(err.message||String(err),'error')}
}
...
function bind(){
  ...
  $('notificationsBtn')?.addEventListener('click',()=>toggleNotifications());$('notificationsClearBtn')?.addEventListener('click',()=>{notificationHistory=[];renderNotifications();toggleNotifications(false)});document.addEventListener('mousedown',e=>{const d=$('notificationDrawer'),b=$('notificationsBtn');if(d&&!d.classList.contains('hidden')&&!d.contains(e.target)&&!b?.contains(e.target))toggleNotifications(false)});
  ...
  $('minimizeBtn').addEventListener('click',api.minimize);$('maximizeBtn').addEventListener('click',api.toggleMaximize);$('closeBtn').addEventListener('click',api.close);$('operationMinimizeBtn')?.addEventListener('click',minimizeOperation);$('operationTaskPill')?.addEventListener('click',restoreOperation);$('sidebarCollapseBtn')?.addEventListener('click',async()=>{const collapsed=!document.body.classList.contains('sidebar-collapsed');document.body.classList.toggle('sidebar-collapsed',collapsed);try{const cfg=await api.saveSettings({launcher:{sidebarCollapsed:collapsed}});if(appState)appState.config=cfg}catch(_){}});$$('[data-settings-tab]').forEach(btn=>btn.addEventListener('click',()=>{if($('settingsSearch'))$('settingsSearch').value='';setSettingsGroup(btn.dataset.settingsTab);applySettingsSearch();}));
  ...
  $('clearCacheBtn')?.addEventListener('click',clearPackCacheAction);$('modsAuditBtn')?.addEventListener('click',()=>refreshModAudit(true));
  ...
  $('developerBackupSourceBtn')?.addEventListener('click',async()=>{try{const r=await api.developerBackupSource();toast(`Backup de SIEGE creado · ${r.copied||0} mods.`, 'success')}catch(err){toast(err.message||String(err),'error')}});$('developerOpenRepoBtn')?.addEventListener('click',()=>{const repo=$('developerRepo')?.value.trim();if(repo?.includes('/'))api.openExternal(`https://github.com/${repo}`);else toast('Configurá el repositorio primero.','warn')});$('developerUnlockBtn').addEventListener('click',unlockDeveloper);$('developerPassword').addEventListener('keydown',e=>{if(e.key==='Enter')unlockDeveloper()});$('developerChooseSource').addEventListener('click',chooseDeveloperSource);$('developerChooseTest')?.addEventListener('click',chooseDeveloperTest);$('developerSyncTestBtn')?.addEventListener('click',syncDeveloperTest);$('developerOpenTestBtn')?.addEventListener('click',()=>api.developerOpenTest().catch(err=>toast(err.message||String(err),'error')));$('developerLaunchTestBtn')?.addEventListener('click',async()=>{try{await api.developerLaunchTest();toast('test-1 iniciado con la configuración actual.','success')}catch(err){toast(err.message||String(err),'error')}});$('developerPreviewBtn')?.addEventListener('click',previewDeveloperPack);$('developerPublishBtn').addEventListener('click',publishDeveloperPack);$('developerCompareTestBtn')?.addEventListener('click',compareDeveloperTest);$('developerPromoteAllBtn')?.addEventListener('click',promoteAllTestMods);$('developerSaveCurseBtn').addEventListener('click',saveCurseForgeKey);$('developerChangePasswordBtn').addEventListener('click',changeDeveloperPassword);$('developerLockBtn').addEventListener('click',async()=>{try{renderDeveloper(await api.developerLock());toast('Modo desarrollador bloqueado.','success')}catch(err){toast(err.message||String(err),'error')}});
