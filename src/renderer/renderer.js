...
async function saveSettings(silent=false){
  ...
  try{const config=await api.saveSettings(patch);appState.config=config;fillBaseState({...appState,config});if(!silent){$('settingsSaved').textContent='GUARDADO';setTimeout(()=>$('settingsSaved').textContent='',2200);toast('Ajustes guardados.','success');}}catch(err){if(!silent)toast(err.message||String(err),'error')}
}
...
function bind(){
  ...
  $$('#page-settings input, #page-settings select, #page-settings textarea').forEach(el=>{el.addEventListener('input',queueSettingsSave);el.addEventListener('change',queueSettingsSave)});
  ...
  $('clearCacheBtn')?.addEventListener('click',clearPackCacheAction);$('modsAuditBtn')?.addEventListener('click',()=>refreshModAudit(true));$('openLastLogMaintenanceBtn')?.addEventListener('click',()=>api.openLogs());
