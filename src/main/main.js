...
function portableSettings(config) {
  return {
    ...
    launcher:{ hideOnGameStart:Boolean(config.launcher?.hideOnGameStart), refocusOnGameExit:config.launcher?.refocusOnGameExit!==false, background:config.launcher?.background||'frontline', backgroundMode:config.launcher?.backgroundMode||'fixed', backgroundBrightness:Math.max(.7,Math.min(1.2,Number(config.launcher?.backgroundBrightness)||1)), theme:config.launcher?.theme||'aurora', density:config.launcher?.density||'comfortable', glassEffects:config.launcher?.glassEffects!==false, scanlines:Boolean(config.launcher?.scanlines), noise:Boolean(config.launcher?.noise), reducedMotion:Boolean(config.launcher?.reducedMotion), uiScale:config.launcher?.uiScale||'normal', accent:String(config.launcher?.accent||''), accent2:String(config.launcher?.accent2||''), cardRadius:Number(config.launcher?.cardRadius||16), clipsDirectory:String(config.launcher?.clipsDirectory||''), galleryView:{query:String(config.launcher?.galleryView?.query||'').slice(0,120),type:['all','image','video'].includes(config.launcher?.galleryView?.type)?config.launcher.galleryView.type:'all',sort:['newest','oldest','name','size'].includes(config.launcher?.galleryView?.sort)?config.launcher.galleryView.sort:'newest'}, profiles:Array.isArray(config.launcher?.profiles)?config.launcher.profiles.slice(-8).map((profile,index)=>({id:String(profile?.id||('profile-'+index)).slice(0,64),name:String(profile?.name||('Perfil '+(index+1))).slice(0,32),patch:{launcher:{theme:String(profile?.patch?.launcher?.theme||'aurora'),background:String(profile?.patch?.launcher?.background||'frontline'),backgroundMode:String(profile?.patch?.launcher?.backgroundMode||'fixed'),density:String(profile?.patch?.launcher?.density||'comfortable'),uiScale:String(profile?.patch?.launcher?.uiScale||'normal'),accent:String(profile?.patch?.launcher?.accent||''),accent2:String(profile?.patch?.launcher?.accent2||''),cardRadius:Number(profile?.patch?.launcher?.cardRadius||16),glassEffects:profile?.patch?.launcher?.glassEffects!==false,scanlines:Boolean(profile?.patch?.launcher?.scanlines),noise:Boolean(profile?.patch?.launcher?.noise),reducedMotion:Boolean(profile?.patch?.launcher?.reducedMotion)}}})):[], startPage:config.launcher?.startPage||'home', rememberLastPage:Boolean(config.launcher?.rememberLastPage), autoConnectivityCheck:config.launcher?.autoConnectivityCheck!==false, closeToTray:config.launcher?.closeToTray!==false, startWithSystem:Boolean(config.launcher?.startWithSystem), startMinimized:Boolean(config.launcher?.startMinimized), nativeNotifications:config.launcher?.nativeNotifications!==false, sidebarCollapsed:Boolean(config.launcher?.sidebarCollapsed), lastSeenVersion:String(config.launcher?.lastSeenVersion||''), crashStreak:Number(config.launcher?.crashStreak||0) },
...
function sanitizeImportedSettings(raw={}) {
  ...
  if(raw.launcher&&typeof raw.launcher==='object') {
    const l=raw.launcher; const themes=new Set(['aurora','tactical','crimson','frost','obsidian','dvn','dvn-ember','dvn-sand','dvn-night','classic','dominion','nusia','ember','clean','neon','verdant','monolith']); const backgrounds=new Set(['frontline','night','canyon','anniversary','cyborg','dummies','orbit','laststand','vought','nightop','rooftop','tempest','urban','dvn-training','dvn-outpost','dvn-warehouse','dvn-neon']);
    const hexColor = (value) => /^#[0-9a-f]{6}$/i.test(String(value || '')) ? String(value) : '';
    const profileThemes=new Set(['aurora','tactical','crimson','frost','obsidian','dvn','dvn-ember','dvn-sand','dvn-night','classic','dominion','nusia','ember','clean','neon','verdant','monolith']); const profileBackgrounds=new Set(['frontline','night','canyon','anniversary','cyborg','dummies','orbit','laststand','vought','nightop','rooftop','tempest','urban','dvn-training','dvn-outpost','dvn-warehouse','dvn-neon']); const profiles=Array.isArray(l.profiles)?l.profiles.slice(-8).map((profile,index)=>{const p=profile?.patch?.launcher||{};return {id:String(profile?.id||`profile-${index}`).slice(0,64),name:String(profile?.name||`Perfil ${index+1}`).trim().slice(0,32),patch:{launcher:{theme:profileThemes.has(String(p.theme))?String(p.theme):'aurora',background:profileBackgrounds.has(String(p.background))?String(p.background):'frontline',backgroundMode:['fixed','randomStartup','rotate5','rotate15'].includes(String(p.backgroundMode))?String(p.backgroundMode):'fixed',backgroundBrightness:Math.max(.7,Math.min(1.2,Number(p.backgroundBrightness)||1)),density:['comfortable','compact'].includes(String(p.density))?String(p.density):'comfortable',uiScale:['small','normal','large'].includes(String(p.uiScale))?String(p.uiScale):'normal',accent:hexColor(p.accent),accent2:hexColor(p.accent2),cardRadius:Math.max(8,Math.min(28,Number(p.cardRadius)||16)),glassEffects:p.glassEffects!==false,scanlines:Boolean(p.scanlines),noise:Boolean(p.noise),reducedMotion:Boolean(p.reducedMotion)}}};}):[];
